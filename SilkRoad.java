...
public class SilkRoad
{
    ...
    public void moveRobot(int location, int meters){
        for(Robot r: roadRobot){
            if(r.getPosition() == location){
                if(r.getType()=="neverback" && meters >= 0){
                    r.advance(meters);
                }
                else if(r.getType()!="neverback"){
                    r.advance(meters);
                }
                if(r.getType()=="onlyMovePair" && meters % 2 == 0){
                    r.advance(meters);
                }
    ...
    public void checkTypes(Robot r, Store c){
        if(r.getType()=="normal" && c.getType()=="normal"){
            r.setTenges(c.getTenges());
            r.setProfit(c.getTenges());
            c.empty();
        }
        else if(r.getType()=="normal" && c.getType()=="figther" && r.getTenges() > c.getTenges()){
            r.setTenges(c.getTenges());
            r.setProfit(c.getTenges());
            c.empty();
        }
        else if(r.getType()=="tender" && c.getType()=="normal"){
            r.setTenges(c.getTenges()/2);
            r.setProfit(c.getTenges()/2);
            c.setTenges(c.getTenges() + c.getTenges()/2);
        }
        else if(r.getType()=="tender" && c.getType()=="figther" && r.getTenges() > c.getTenges()){
            r.setTenges(c.getTenges()/2);
            r.setProfit(c.getTenges()/2);
            c.setTenges(c.getTenges() + c.getTenges()/2); 
        } else if (r.getType()=="normal" && c.getType()=="superStore"){
            r.setTenges(c.getTenges());
            r.setProfit(c.getTenges());
        } else if (r.getType()=="tender" && c.getType()=="superStore"){
            r.setTenges(c.getTenges()/2);
            r.setProfit(c.getTenges()/2);
        }
    }
